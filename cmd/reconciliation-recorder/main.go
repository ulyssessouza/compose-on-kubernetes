...
func run(stacksToProceed []string, opts *options) error {
	...
	if len(stacksToProceed) == 0 {
		allStacks, err := stacks.ComposeV1alpha3().Stacks(opts.namespace).List(metav1.ListOptions{IncludeUninitialized: true})
		if err != nil {
			return err
		}
...
func processStack(stacks clientset.Interface, k8sclient k8sclientset.Interface, name, namespace, outdir string) error {
	stack, err := stacks.ComposeV1alpha3().Stacks(namespace).Get(name, metav1.GetOptions{IncludeUninitialized: true})
	if err != nil {
		return err
	}
